...
describe('Auth slice passwordless and admin flows', () => {
  ...
  it('logs in regular users with a Google credential and persists the token', async () => {
    ...
    vi.mocked(authService.loginWithGoogle).mockResolvedValue({ token: 'google-token', user });
  ...
  it('registers without persisting a token or creating a session', async () => {
    ...
    vi.mocked(authService.register).mockResolvedValue(response);
  ...
  it('supports admin password login separately', async () => {
    ...
    vi.mocked(authService.adminLogin).mockResolvedValue({ token: 'admin-token', user });
    ...
    if (!state.user) throw new Error('Expected an authenticated admin user');
  ...
  it('clears the session on logout', () => {
    ...
    const state = authReducer(
      { ...initialState, user: { id: '1', name: 'Member', email: 'member@example.com' }, token: 'token', error: 'error' },
      logoutAction()
    );
