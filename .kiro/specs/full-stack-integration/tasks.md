...
- [ ] 5. Authentication service (`authService.js`)
  - [~] 5.1 Implement `login(email, password)` → `POST /api/auth/login`
  - [~] 5.2 Implement `register(name, email, password)` → `POST /api/auth/register`
  - [~] 5.3 Implement `forgotPassword(email)` → `POST /api/auth/forgot-password`
  - [~] 5.4 Implement `resetPassword(token, newPassword)` → `POST /api/auth/reset-password`

- [ ] 6. Auth Redux slice (`authSlice.js`)
  - [~] 6.1 On module load, synchronously rehydrate from `localStorage`: decode the token with `jwtDecode`, reject expired tokens, set the `user` object (`name`, `email`, `role`) and `token` string, or set both to `null` if absent/expired/invalid; set `initialized: true`
  - [~] 6.2 Implement `loginThunk`: call `authService.login`, store token in `localStorage`, return `{ token, user }` on success; call `rejectWithValue(err)` on failure
  - [~] 6.3 Implement `registerThunk`: same pattern as `loginThunk` using `authService.register`
  - [~] 6.4 Implement `logoutAction` reducer: remove token from `localStorage`, set `auth.user` and `auth.token` to `null`, clear `auth.error`
  - [~] 6.5 Wire `extraReducers` for `loginThunk` and `registerThunk`: pending → `loading: true, error: null`; fulfilled → `loading: false, token, user`; rejected → `loading: false, error: action.payload`

- [ ] 7. Auth page wiring
  - [~] 7.1 `LoginPage.jsx` — remove fake localStorage auth logic and hardcoded credential quick-fill buttons; dispatch `loginThunk({ email, password })` on form submit; display `auth.error` on failure
  - [~] 7.2 `RegisterPage.jsx` — dispatch `registerThunk({ name, email, password })` on valid form submit; display `auth.error` on failure
  - [~] 7.3 `ForgotPasswordPage.jsx` — call `authService.forgotPassword(email)` on form submit; show a success message on resolve; show the error string on reject

- [ ] 8. Route guard components and App.jsx wiring
  - [~] 8.1 Create `client/src/components/common/ProtectedRoute.jsx`: read `{ token, initialized }` from `s.auth`; render loading spinner while `!initialized`; redirect to `/login` when no token; render `<Outlet />` otherwise
  - [~] 8.2 Create `client/src/components/common/AdminRoute.jsx`: same guard logic plus redirect to `/` when token is present but `user.role !== 'admin'`
  - [~] 8.3 Update `App.jsx`: wrap `/account`, `/wishlist`, `/purchase-history`, `/borrowing-history`, `/movie-request`, `/notifications`, and `/book-confirm` with `<ProtectedRoute />`
  - [~] 8.4 Update `App.jsx`: wrap `/admin`, `/admin/movies`, `/admin/books`, and `/admin/electronics` with `<AdminRoute />`
  - [~] 8.5 Update `Navbar.jsx`: replace all localStorage `ahadu_logged_in` / `auth-change` logic with `useSelector((s) => s.auth)` and `useSelector((s) => s.notification)`; show Sign In / Register when no token; show avatar dropdown and logout button when token is present; dispatch `logoutAction` and navigate to `/` on logout; show notification badge when `unreadCount > 0`

- [ ] 9. Book service (`bookService.js`)
  - [~] 9.1 Implement `getBooks(params)` → `GET /api/books` with query params
  - [~] 9.2 Implement `getBook(id)` → `GET /api/books/:id`
  - [~] 9.3 Implement `borrowBook(id)` → `POST /api/books/:id/borrow`
  - [~] 9.4 Implement `reserveBook(id)` → `POST /api/books/:id/reserve`
  - [~] 9.5 Implement `getBookReviews(id, params)` → `GET /api/books/:id/reviews`
  - [~] 9.6 Implement `createBookReview(id, payload)` → `POST /api/books/:id/reviews`

- [ ] 10. Book Redux slice (`bookSlice.js`)
  - [~] 10.1 Define initial state: `books: [], selectedBook: null, reviews: [], loading: false, error: null, pagination: { totalItems: 0, totalPages: 0, currentPage: 1, limit: 12 }`
  - [~] 10.2 Implement `fetchBooks(params)` thunk → `bookService.getBooks`; on fulfilled map server envelope `{ data, totalCount, page, totalPages, limit }` to slice state
  - [~] 10.3 Implement `fetchBook(id)` thunk → `bookService.getBook`; on fulfilled set `selectedBook`
  - [~] 10.4 Implement `borrowBook(id)` thunk → `bookService.borrowBook`; on fulfilled return payload for page to display
  - [~] 10.5 Implement `reserveBook(id)` thunk → `bookService.reserveBook`; on fulfilled return payload
  - [~] 10.6 Implement `fetchBookReviews(id, params)` thunk → `bookService.getBookReviews`; on fulfilled set `reviews`
  - [~] 10.7 Implement `createBookReview(id, payload)` thunk → `bookService.createBookReview`; on fulfilled append to `reviews`
  - [~] 10.8 Wire all pending/rejected cases: pending → `loading: true`; rejected → `loading: false, error: action.payload`

- [ ] 11. Book page wiring
  - [~] 11.1 `BookCenterPage.jsx` — dispatch `fetchBooks` with default params on mount; dispatch again on filter/page change; replace hardcoded book array with `book.books` from store; use `book.pagination` for pagination controls; show loading skeleton while `book.loading`; show error banner with retry on `book.error`
  - [~] 11.2 `BookDetailPage.jsx` — dispatch `fetchBook(id)` on mount using URL param; dispatch `borrowBook` / `reserveBook` thunks on button click; show server response message or fallback; wire `ReviewsCommentsSection` to dispatch `fetchBookReviews(id)` on mount and `createBookReview(id, payload)` on authenticated review submit
  - [~] 11.3 `BookConfirmPage.jsx` — read `?action=` and `?id=` query params; dispatch `fetchBook(id)` on mount; call `bookService.borrowBook(id)` or `bookService.reserveBook(id)` on confirm; display loading skeleton while loading; on success show server-returned details (due date / reservation expiry); on error display error message and keep form visible for retry

- [ ] 12. Movie service (`movieService.js`)
  - [~] 12.1 Implement `getMovies(params)` → `GET /api/movies`
  - [~] 12.2 Implement `getMovie(id)` → `GET /api/movies/:id`
  - [~] 12.3 Implement `getMovieReviews(id, params)` → `GET /api/movies/:id/reviews`
  - [~] 12.4 Implement `createMovieReview(id, payload)` → `POST /api/movies/:id/reviews`
  - [~] 12.5 Implement `submitMovieRequest(payload)` → `POST /api/movie-requests`
  - [~] 12.6 Implement `getUserMovieRequests()` → `GET /api/users/me/movie-requests`
  - [~] 12.7 Implement `cancelMovieRequest(id)` → `DELETE /api/movie-requests/:id`

- [ ] 13. Movie Redux slice (`movieSlice.js`)
  - [~] 13.1 Define initial state: `movies: [], selectedMovie: null, reviews: [], loading: false, error: null, pagination: { totalItems: 0, totalPages: 0, currentPage: 1, limit: 12 }`
  - [~] 13.2 Implement `fetchMovies(params)` thunk; on fulfilled map paginated envelope to slice state
  - [~] 13.3 Implement `fetchMovie(id)` thunk; on fulfilled set `selectedMovie`
  - [~] 13.4 Implement `fetchMovieReviews(id, params)` thunk; on fulfilled set `reviews`
  - [~] 13.5 Implement `createMovieReview(id, payload)` thunk; on fulfilled append to `reviews`
  - [~] 13.6 Wire all pending/rejected cases
